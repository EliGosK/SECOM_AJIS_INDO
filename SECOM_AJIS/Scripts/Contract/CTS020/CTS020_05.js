...
function SetCTS020_05_SectionMode(isview) {
...
function SetCTS020_05_EnableSection(enable) {
...
function AddEmail(mails, hilight, frompopup) {
    ...
    /* --- Check and Add event --- */
    /* --------------------------- */
    call_ajax_method_json("/Contract/CTS020_CheckBeforeAddEmailAddress", obj, function (result, controls) {
        /* --- Higlight Text --- */
        /* --------------------- */
        if (hilight == true) {
...
function DeleteEmail(rid) {
    ...
    call_ajax_method_json("/Contract/CTS020_RemoveEmailAddress", obj, function (result, controls) {
        DeleteRow(gridEmailList, rid);
    });
}

function CTS020_05_InitialGrid() {
    gridEmailList = $("#gridEmailList").LoadDataToGridWithInitial(0, false, false,
                                "/Contract/CTS020_GetEmailList",
                                "",
                                "tbt_DraftSaleEmail", false);
...
function SearchEmailAddress() {
    $("#dlgBox").OpenCMS060Dialog("CTS020");
}
function CMS060Response(doEmailAddress) {
    ...
    if (gridEmailList != undefined && doEmailAddress != undefined) {
        ...
        //setTimeout('AddEmailFromPopup(doEmailAddress, false, true);', 1000);
    }
}
...
function AddEmailFromPopup(mails, hilight, frompopup) {
    ...
    /* --- Check and Add event --- */
    /* --------------------------- */
    call_ajax_method_json("/Contract/CTS020_AddEmailAddressFromPopup", obj, function (result, controls) {
        /* --- Higlight Text --- */
        /* --------------------- */
        if (hilight == true) {
            VaridateCtrl(["EmailAddress"], controls);
