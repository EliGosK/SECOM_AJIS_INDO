...
function initScreen() {
    ajax_method.CallScreenController("/inventory/IVS120_InitParam", "", function (result, controls) {
        if (result) {
    ...
    if ($("#InstGrid").length > 0) {
        InstGrid = $("#InstGrid").InitialGrid(ROWS_PER_PAGE_FOR_SEARCHPAGE, false, "/inventory/IVS120_InstGrid");
            ...
            if (CheckFirstRowIsEmpty(InstGrid, false) == false) {
                for (var i = 0; i < InstGrid.getRowsNum(); i++) {
                    ...
                    BindGridButtonClickEvent("btnSelect", row_id, function (rid) {
                        ...
                        ajax_method.CallScreenController("/inventory/IVS120_beforeAddElem", { Cond: obj }, function (result, controls) {
                            if (result) {
                                ...
                                obj.row_id = row_id;
                                ajax_method.CallScreenController("/inventory/IVS120_UpdateRowIDElem", obj, function (result, controls) {
                                    //alert("Updateed");
                                });
                                ...
                                BindGridButtonClickEvent("btnRemove", row_id, function (rid) {
                                    ...
                                    ajax_method.CallScreenController("/inventory/IVS120_DelElem", {
    ...
    if ($("#EliminateInst").length > 0) {
        StockGrid = $("#EliminateInst").InitialGrid(0, false, "/inventory/IVS120_TransferStockInstGrid");
    }
}
...
function initButton() {
    ...
    $("#btnSearch").click(IVS120_Search);
}

function IVS120_Search() {
    ...
    $("#InstGrid").LoadDataToGrid(InstGrid, ROWS_PER_PAGE_FOR_SEARCHPAGE, false, "/inventory/IVS120_SearchInventoryInstrument", obj, "dtSearchInstrumentListResult", false
...
function cmdRegister() {
    ...
    ajax_method.CallScreenController("/inventory/IVS120_cmdReg", obj, function (result, controls) {
        if (controls != undefined) {
...
function cmdConfirm() {
    ...
    ajax_method.CallScreenController("/inventory/IVS120_cmdConfirm", null, function (result, controls) {
        var ctrlLst = new Array();
        if (controls != undefined) {
            for (var idx = 0; idx < controls.length; idx++) {
                ctrlLst.push(controls[idx]);
            }
        }
...
function downloadDocumentReport() {
    ajax_method.CallScreenController("/Inventory/IVS120_CheckExistFile", "", function (result) {
        if (result == 1) {
            var key = ajax_method.GetKeyURL(null);
            var url = ajax_method.GenerateURL("/Inventory/IVS120_DownloadPdfAndWriteLog?k=" + key);
