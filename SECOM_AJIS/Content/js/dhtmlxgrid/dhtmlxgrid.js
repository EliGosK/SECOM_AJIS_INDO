...
function dhtmlXGridObject(id) {
    ...
    this.setSizes = function() {
        ...
        if (this._ahgr) {
            if (this._ahgrMA) newHeight = this.entBox.parentNode.clientHeight - headerHeight - footerHeight;
                ...
                newHeight = this.obj.offsetHeight + (isHScroll ? scrfix : 0); if (this._ahgrM) {
    ...
    this.setInitWidths = function(wp) {
        ...
        if (_isFF) {
            for (var i = 0; i < this.initCellWidth.length; i++)
                if (this.initCellWidth[i] != "*") this.initCellWidth[i] = parseInt(this.initCellWidth[i])
        }
    };
    ...
    this.selectCell = function(r, cInd, fl, preserve, edit, show) {
        ...
        if (r._childIndexes) var c = r.childNodes[r._childIndexes[cInd]];
            ...
            var c = r.childNodes[cInd]; if (!c) c = r.childNodes[0];
    ...
    this._getNextCell = function(acell, dir, i) {
        ...
        if (this._tabOrder) {
            ...
            if (typeof i != "undefined")
                if (i < 0) acell = this._nextRowCell(arow, dir, Math.abs(i) - 1);
                else
    ...
    this.getRowIndex = function(row_id) {
        for (var i = 0; i < this.rowsBuffer.length; i++)
            if (this.rowsBuffer[i] && this.rowsBuffer[i].idd == row_id) return i;
        return -1
    };
    ...
    this.changeRowId = function(oldRowId, newRowId) {
        ...
        for (var i = 0; i < row.childNodes.length; i++)
            if (row.childNodes[i]._code) row.childNodes[i]._code = this._compileSCL(row.childNodes[i]._val, row.childNodes[i]);
        if (this._mat_links && this._mat_links[oldRowId]) {
            var a = this._mat_links[oldRowId];
            delete this._mat_links[oldRowId];
            for (var c in a)
                for (var i = 0; i < a[c].length; i++) this._compileSCL(a[c][i].original, a[c][i])
        };
    ...
    this.getColIndexById = function(id) {
        for (var i = 0; i < this.columnIds.length; i++)
            if (this.columnIds[i] == id) return i
    };
    ...
    this.serialize = function() {
        ...
        if (this._mathSerialization) this._agetm = "getMathValue";
            ...
            this._agetm = "getValue"; if (this._sUDa && this.UserData["gridglobaluserdata"]) {
...
dhtmlXGridObject.prototype = {
    ...
    _unsetRowHover: function(e, c) {
        if (c) that = this;
            ...
            that = this.grid; if ((that._lahRw) && (that._lahRw != c)) {
    ...
    _launchCommands: function(arr) {
        for (var i = 0; i < arr.length; i++) {
            ...
            for (var j = 0; j < arr[i].childNodes.length; j++)
                if (arr[i].childNodes[j].nodeType == 1) args[args.length] = arr[i].childNodes[j].firstChild.data;
    ...
    getAllRowIds: function(separator) {
        var ar = [];
        for (var i = 0; i < this.rowsBuffer.length; i++)
            if (this.rowsBuffer[i]) ar.push(this.rowsBuffer[i].idd);
        ...
        for (var i = 0; i < data[0].length; i++) {
            ...
            if (typeof data[0][i] == "object") w.appendChild(data[0][i]);
                ...
                if (this.forceDivInHeader) w.innerHTML = "<div class='hdrcell'>" + (data[0][i] || "&nbsp;") + "</div>";
                    ...
                    w.innerHTML = (data[0][i] || "&nbsp;"); if ((data[0][i] || "").indexOf("#") != -1) {
    ...
    setColumnExcellType: function(colIndex, type) {
        for (var i = 0; i < this.rowsBuffer.length; i++)
            if (this.rowsBuffer[i] && this.rowsBuffer[i].tagName == "TR") this.changeCellType(this.rowsBuffer[i], colIndex, type);
    ...
    forEachRow: function(custom_code) {
        for (var a in this.rowsAr)
            if (this.rowsAr[a] && this.rowsAr[a].idd) custom_code.apply(this, [this.rowsAr[a].idd])
    },
        ...
        for (var i = 0; i < rows.length; i++) {
            ...
            if (this.rowsAr[id] && this.rowsAr[id].tagName != "TR") {
                ...
                if (this._refresh_mode[1] == "top") this.rowsBuffer.unshift(dadd);
                    ...
                    this.rowsBuffer.push(dadd); if (this._h2) {
                    reset = true;
    ...
    _get_cell_value: function(row, ind, method) {
        ...
        return this.cells3(row, ind)[this.cells3(row, ind)[method] ? method : "getValue"]()
    },
    ...
    deleteRow: function(row_id, node) {
        ...
        for (var i = 0; i < this.selectedRows.length; i++)
            if (this.selectedRows[i].idd == row_id) this.selectedRows._dhx_removeAt(i);
        if (this._srnd) {
    ...
    getRowById: function(id) {
        ...
        if (row) {
            if (row.tagName != "TR") {
                for (var i = 0; i < this.rowsBuffer.length; i++)
                    if (this.rowsBuffer[i] && this.rowsBuffer[i].idd == id) return this.render_row(i);
